...
import { registerInvestor } from "@/service/api/plantifyService";
...
export default function RegisterInvestor(): JSX.Element {
  ...
  const handleRegisterInvestor = async (
    ...
    try {
      const result = await registerInvestor(actor, data);

      if ("ok" in result) {
        return { success: true, message: "Registration successful!" };
      } else {
        throw new Error(`HTTP error! status: ${result.err}`);
      }
    } catch (error) {
      return {
        success: false,
  ...
  const handleSubmit = async (
    ...
    try {
      const result = await handleRegisterInvestor(values);
